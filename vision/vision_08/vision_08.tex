...
$\chi E^s = 0$ hususunda bir nokta daha, eþlemelerde hata olabileceði için bu
ifade tam olarak çözülemeyebilir. O zaman, ona en yakýn olabilecek çözüme
eriþmeye uðraþýrýz; yani $||\chi E^s||^2$'yi en az kareler baðlamýnda minimize
edecek $E^s$'i hesaplarýz. Bu minimizasyon $E^s$'i $\chi^T\chi$'nin en ufak
özdeðerine tekabül eden özvektörü olarak seçmek ile mümkün olabilir. Tabii
$\chi^T\chi$ özvektör hesabý ile $\chi$ eþsiz deðer ayrýþtýrmasýnýn iliþkisi
...
\begin{pseudocode}
  ...
  \codeline \> Gerekli matrisin yaklaþýk halini bul. \\
  \codeline \> $\chi = \left[\begin{array}{cccc} a^1 & a^2 & \dots & a^n \end{array}\right]^T$'yi hesapla,  ki $a^i = x_1^i \otimes x_2^i$. \\
  \codeline \> $||\chi E^s||$'i minimize edecek þekilde $E^s \in \mathbb{R}^9$'i bul, yani  \\
  \codeline \> $\chi = U_\chi \Sigma_\chi V_\chi^T$ ayrýþtýrmasýnda $V_\chi$'nin 9. kolonunu al, \\
  \codeline \> çünkü o kolon en küçük eþsiz deðere tekabül ediyor. \\
  \codeline \> $E^s$'i tersine yýðma iþlemiyle $3 \times 3$ $E$ vektörüne aç. \\
  \codeline \> Gerekli uzaya yansýtma yap; $E = U diag\{\sigma_1,\sigma_2,\sigma_3\} V^T $.\\
  \codeline \> $E$ belli bir skalara kadar tanýmlý olduðu için $E$'yi normalize edilmiþ \\
  \codeline \> gerekli uzayýna yansýt, $\sigma_1,\sigma_2,\sigma_3$ yerine 1,1,0 deðerleri kullan.\\
  \codeline \> $R,\hat{T}$'yi hesapla. Dört mümkün çözüm $R=UR_Z(\pm \pi/2)V^T$,$\hat{T}=UR_Z(\pm\pi/2)\Sigma U^T$\\
