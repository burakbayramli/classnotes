...
8. derste vazgeçilmez matris (essential matris) konusunu görmüþtük. Þimdi
...
yansýtma matrisleri $P,P'$'yi oluþturabiliriz. Þimdi bu matrisleri
kullanarak üç boyutta gerçek nokta $X$'leri nasýl hesaplarýz, onu görelim
çünkü halen elimizde sadece iki boyutlu imaj noktalarý var, 3D dünya
noktalarý yok. $X$'leri hesaplamak için daha önce gördüðümüz direk lineer
transform metotunun benzerini uygularýz. Bu gerekli çünkü her iki
kameradaki yansýmadan oluþan hatalar, vs. sonucu pür analitik bir denklem
ile uygulanacak bir üçgenleme yöntemini kesin çözmek mümkün
olmayabilir. Alttaki durum ortaya çýkar, 
...
noktasýnýn iki kameradaki deðiþik yansýmalarý var. Bu denklemler
birleþtirilerek bir $AX=0$ sistemi ortaya çýkartýlabilir [1, sf. 312], ve
bu denklem minimizasyon olarak çözülebilir. Çapraz çarpým ile homojen ölçek
faktörünü çýkartýrsak, mesela ilk imaj için
...
lineerdir. Bu sistemden hareketle $AX=0$'daki $A$ þöyle olur, 
...
Her iki imajdan iki denklem alýndý, toplam 4 denklem oldu. Bu denklem yine
SVD ile ya da $AX=b$ þeklinde tekrar düzenlenip 2. derste gördüðümü sözde
ters (pseudoinverse) ile çözülebilir. Altta bu yöntem takip edildi,
...
Test amaçlý olarak bilinen \verb!P1,P2! ile üçgenleme yapalým, sonra elde
edilen üç boyutlu noktalarý kameralara yansýtalým ve baþladýðýmýz imaj
noktalarýna uyuyor mu ona bakalým. 
...
x2real = np.array([[ 0.42 ,  0.537,  0.645,  0.431,  0.538],
                   ...
                   [ 1.   ,  1.   ,  1.   ,  1.   ,  1.   ]])
X = triangulate_point( x1real.T[0], x2real.T[0], P1, P2 )
X /= X[3]
x1 = np.dot(P1[:3],X)
x2 = np.dot(P2[:3],X)
x1 /= x1[2]
...
sonuçlarýn her biri denenmeli. Biz altta tüm bu seçenekleri deneyeceðiz,
yani her biri için bir üç boyutta tekrar oluþturma yapacaðýz, ve sonuçlarý
farklý grafiklerde göstereceðiz.
