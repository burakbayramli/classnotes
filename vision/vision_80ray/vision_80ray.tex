...
p1idx = 44
plt.plot(random_points[mask][p1idx,0], h-random_points[mask][p1idx,1], 'c.')
...
sadece $P$ matrisi olsa, kamera merkezini nasýl hesaplarýz onu
...
kullanacaðýz. Hatýrlatarsak, $P$'nin sözde tersi $P^{\dag}$

$$ P^{\dag} = P^T(PP^T)^{-1}$$

iþlemidir, ki $PP^{\dag} = I$. Ama $PP^T$ çarpýmý sayýsal iyi sonuçlar
vermeyebilir (çarpýmlar çok büyür), endiþeye gerek yok, sayýsal kütüphaneler
bu iþlemi SVD üzerinden çözüyor (çok hýzlý), bkz. 2. ders.
...
benzeyen bir nokta iki üstteki resimde mavi renkli gösterildi. Bu pikselin
ýþýný neye benzer?
...
p1 = [random_points[mask][p1idx,0], h-random_points[mask][p1idx,1], 1]
p1 = np.array(p1)
print p1

X = np.dot(lin.pinv(P),np.array(p1))
X = X / X[3]
XX  = np.copy(X)
...
\includegraphics[width=20em]{vision_80ray_04.png}

Mavi renkli ok kameranýn imaj düzlemine dik (normal) olan vektör. Kýrmýzý
olan ok $p_1$'e iþaret eden ýþýn. 

Þimdi tüm noktalarý yapalým. Altta ilk gösterilen iki nokta (yani iki
noktayý baz alarak sonsuza giden çizgiyi baz alan) ve düzlem (bir nokta,
bir normal) arasýnda kesiþmeyi hesaplayan çaðrýdýr, bkz {\em Çok Boyutlu
  Calculus, Ders 5}. Üstteki gördüðümüz kýrmýzý noktalý pikselleri alýp,
onlarýn ýþýnýný bulacaðýz, sonra bu çizginin $xy$ düzlemi ile kesiþmesini
bulacaðýz. Kamera matrisi $K$'yi biliyoruz, çünkü kamerayý biz kalibre
ettik, detaylar için [2].
...
kullanmamýzýn sebebi eðer yer deðiþtirmeyle beraber dönüþ te (rotation)
durumu varsa (bu örnekte yoktu) bu bilginin de $P$ içinde olacaðýdýr. Bu
durumda üstteki sözde ters ile yine direk bir ýþýn elde edebilirdik. Öteki
türlü çetrefil bir sürü ek hesaplara girmek gerekecekti. Yani tarif
ettiðimiz yaklaþým ile her türlü kamera duruþunu kullanabiliriz.
