...
transformasyon kabaca bakýnca bir yer deðiþtirme (translation) ve ölçekleme
(scaling) içeriyor. Üstteki türden 2D-2D iliþkilerine homografi adý
veriliyor. Aradýðýmýz alttaki türden bir formül,

$$ x' = H x$$

ki orijinal her $x$ noktasý $H$ ile $x'$ haline gelecek. $H$ matrisi
homojen kordinatlarý baz alýr,
...
Bu ölçeklemeye (scaling) izin veren bir katý (rigid) transformasyon
deniyor. ``Katý'' demek $s=1$ ile noktalar arasý mesafeler deðiþmeyecek
demek, $s$ ile tabii ölçekleme olabilir, mesafeler artabilir, ama mesafe
oranlarý yine ayný kalýr, ayrýca döndürme de -rotation- yapýlabilir. Bu
transformasyona yansýtsal (projective) ismi de verilir. Yansýtal
transformasyonun ilgin transformasyondan daha esnek / kuvvetli olduðu
bilinir. 

Not: Mesela ilgin transformasyon literatürde $x' = R x + t$, yani rotasyon
artý yer deðiþimi gibi bir formda da görülebilir, biz homojen sisteme
geçerek her ikisini ayný matris $H$ içinde ve tek çarpým operasyonu ile
gösterebilmiþ oluyoruz. 
...
Yansistal $H$ hesabý için direk lineer transform (direct linear transform
-DLT-) tekniði var. Eldeki tum eslesmeler icin alttaki sistemi yaratiriz, 
...
istiyorum, mesela alttaki Sudoku oyun karesini alýp sadece o olacak þekilde
görmek istiyorum. 
...
noktasýnýn eþit kenarlý bir karenin köþelerine transform edilmesi
demektir. Daha önce yaptýðým gibi $H$ hesaplarým, sonra o bölgedeki tüm
piksellerin alýp hespaladýðým transformasyonu onlara uygularým,
...
çözüm þaþýrtýcý derecede basit. 

[4]'ün çözümü þöyle özetlenebilir: bir video'yu baþtan itibaren kare kare
iþlerken, her karede ilginç köþe noktalarý (Harris tekniði ile) buluruz, ve
bu noktalarýn bir sonraki resimdeki eþlerini elde ederiz. Tüm eþlemeleri
kullanarak bir homografi / transformasyon hesaplarýz, bu transformasyon
matrisi içinde $x,y$ deðiþimi, yani taþýnma, ve $a$ açýsý ki döndürme
bilgisi vardýr. Bu bilgileri $dx,dy,da$ olarak biriktiririz. 
...
seriden olan farklarýný buluruz, ve her kare için bu farký alýp, bir önceki
kare üzerinde yamultma yaparak onu ``düzeltiriz''.

Özetlemek gerekirse üstte gösterdiðimiz türde video'larda "beklenen" bir
nokta eþleþmesi vardýr, düz yürüyoruz, kamera ileri dönük, ortadaki
pikseller dýþa doðru eþleþmeli, saðdakiler daha saða doðru, vs.  Bu
beklentiyi hareketli ortalama ile hesaplamak mümkün, ve ondan olan
sapmalarý kameranýn istenmeyen titremesi olarak algýlýyoruz, ve
düzeltiyoruz.

\inputminted[fontsize=\footnotesize]{python}{vidstab.py}

Üstteki kodu [1] üzerinde uygularsak stabilizasyon yapýldýðýný
göreceðiz. Sonuç [2]'de. 

C++ kodu \verb!vidstab.cpp!'de bulunabilir.

Canlý Zamanda (Real-Time) Stabilizasyon

[4]'un tekniði toptan (batch) iþleyen bir teknik, ortalama alýnmasý,
düzeltme yapýlmasý için video'nun baþtan sona iþlenmesi gerekiyor, ve
geriye dönülmesi gerekiyor. Düzeltme iþlemini canlý olarak yapamaz mýyýz?

Bu mümkün olmalý; yürüyen ortalama için {\em Zaman Serileri ve Finans,
  ARIMA, ARCH, GARCH, Periyotlar, Yürüyen Ortalama} yazýsýnda iþlenen üstel
aðýrlýklý hareketli ortalama kullanýlabilir. Bu ortalamanýn özyineli
(recursive) formu da vardýr,
...
hesaplariz, bunlarin kumulatif toplamini da anlýk hesaplarýz (kolay). Bu
kumulatif $x,y,a$'yý üstteki tanýmda $g_t$ olarak formüle veririz, en son
ortalama her zaman $z_t$ içinde olacaktýr. Bu ortalamanýn kumulatif olandan
farký, ``sapmasý'' her kare üzerinde düzeltme amacý ile
kullanýlabilir. Kodlamasý okuyumuza ödev olsun!
...
[5] Bayramli, {\em OpenCV 3.0}, \url{http://sayilarvekuramlar.blogspot.com/2017/03/opencv-30.html}
