...
$$ \hat{V}_y = g_{xy}\hat{V}g_{xy}^{-1}Y(t) = ad_{g_{xy}}(\hat{V})$$
...
\hline
   & Rotasyon SO(3) & Katý gövde SE(3) \\
\hline
  Matris temsili &  R $\in$ GL(3): $R^TR=I$, $\det(R)=1$ & 
  $g = \left[\begin{array}{rr} R & T \\ 0 & 1 \end{array}\right]$ \\
\hline
 3-D kordinatlar & $X=RX_0$ & $X = RX_0 + T$ \\
\hline
 Tersi & $R^{-1}=R^T$ & 
 $g^{-1} = \left[\begin{array}{rr} R^T & -R^T \\ 0 & 1 \end{array}\right]$\\
\hline
 Üstel temsil & $R = \exp(\hat{w})$ & $g=\exp(\hat{\xi})$ \\
\hline
