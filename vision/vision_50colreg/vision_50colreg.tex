...
Bir resimdeki en yaygýn renkleri bulmak için [2],
...
Þimdi resmin yaygýn renklerinden birinin (üstteki renklerde en saðdaki kýrmýzý
mesela) resmin hangi piksellerine en yakýn olduðunu bulalým. Basit uzaklýk
ölçüsü kullanarak H,S,V renk üçlüsü üzerinden bir uzaklýk hesaplayacaðýz, belli
bir eþik deðeri altýnda olan tüm pikselleri mavi ile göstereceðiz.
...
Uzaklýk için özellikle R,G,B deðil H,S,V kullandýk çünkü bu renk temsilinin
uzaklýk hesaplarýnda daha iyi iþlediði biliniyor. 

Bölgeler Eþit mi?

Ýki imaj bölgesinin birbiriyle ayný mý farklý mý olduðu sorusu imaj gruplamasý
(segmentation) ya da kümelemesi için önemli bir soru. Elimizde iki piksel grubu
var, birinin diðerine ait olduðunu nasýl bileceðiz?

Ýlginç bir çözüm þu olabilir; piksel deðerlerinin bir olasýlýk daðýlýmýndan
örneklendiðini düþünmek, ve her iki bölgenin ayný daðýlýmdan gelip gelmediðini
kontrol etmek [1, sf. 99].

Diyelim ki belli bir düzeni, yapýsý olan bir imaj bölgesi ayný / sabit bir gri
deðerinin, istatistiki olarak baðýmsýz, 0-deðerli Gaussian'dan gelen bir gürültü
eklenmiþ hali. Elimizde iki bölge var, $R_1,R_2$, içlerinde sýrasýyla $m_1,m_2$
tane piksel deðeri var. Ýki hipotez mümkün,

$H_0$: Her iki bölge ayný objeye ait. Bu durumda her iki bölgenin tüm gri renk
deðerleri tek bir Gaussian'dan örneklenmiþtir, ki bu Gaussian $(\mu_0,\sigma_0^2)$
olsun.

$H_1$: Ýmaj bölgeleri / pikselleri farklý objelere ait. Bu durumda her piksel
grubu ayrý Gaussian daðýlýmýndan geliyor, 1. bölge $(\mu_1,\sigma_1^2)$,
2. bölge $(\mu_2,\sigma_2^2)$.

Çoðunlukla bu parametreler bilinmez, maksimum olurluk (likelihood) kullanýlarak
veriden kestirilirek hesaplanýr,

$$ \hat{\mu} = \frac{1}{n} \sum _{i=1}^{n}g_i $$

$$ \hat{\sigma} = \frac{1}{n} \sum _{i=1}^{n} (g_i-\hat{\mu})^2 $$

Bunlar temel istatistikten bildiðimiz þeyler.  Simdi herhangi bir $\mu,\sigma$
için bir piksel deðeri $g_i$'in olasýlýðý

$$ p(g_i) = \frac{1}{\sqrt{2\pi} \sigma_0} \exp (- (g_i-\mu_0)^2 / 2\sigma^2)  $$

$H_0$ altýnda tüm pikseller ayný daðýlýmdan geliyor, o zaman tüm verileri
gözönüne alan ortak daðýlým,
...
Çarpýmýn $\exp$ içine nüfuz edince toplam olduðuna dikkat; ayrýca $\exp$
içindeki $\mu_0,\sigma_0$ nereye gitti? $\mu_0,\sigma_0$ yerine onlarýn maksimum
olurluk kestirme formüllerini geçirince iptal olan terimlerden arta kalanýn
üstteki sonuç olduðunu görebiliriz.

$H_1$ için durum farklý. Burada $m_1$ tane piksel $(\mu_1,\sigma_1^2)$
daðýlýmýna sahip 1. bölgeye, $m_2$ tane piksel $(\mu_2,\sigma_2^2)$ daðýlýmýna
sahip 2. bölgeye ait. Bu hipotez altýnda ortak daðýlým,
...
Artýk olurluk oranýný hesaplayabiliriz,

$$ L = \frac{p(g_1,g_2,.. | H_1)}{p(g_1,g_2,.. | H_0)}$$

$$ = \frac{\sigma^{m_1+m_2}}{\sigma^m_1 \sigma^m_2} $$

$\sigma_0, \sigma_1 , \sigma_2$ sýrasýyla tüm $m_1+m_2$ piksel, 1. bölgeden
$m_1$ piksel, 2. bölgeden $m_2$ tane piksel kullanarak veriden yine maksimum
olurluk ile kestirilecek. Eðer üstteki oran belli bir eþik deðerinin altýnda ise
bunu bölgelerin birbirine çok benzediðine yönelik bir kanýt olarak kabul
edeceðiz, üstte ise farklý bölgeler olduðuna kanaat getireceðiz.

Örnek

Alttaki imajlarda iki bölge seçtik, iki bölge þunlar, biri iç dikdörtgen diðeri
dýþ dikdörtgenin kesiþme dýþýnda kalan pikselleri (eðer kesiþme yoksa her iki
dikdörtgen ayrý ayrý),
...
Olurluk oranýnýn log'unu alarak hesap yapýnca
...
Ýkinci resimde her iki dikdörtgen ayný direðin üzerinde, yani ayný obje
üzerindeler. Bu durumda oranýn daha düþük olmasýný bekleriz,
...
Eðer renkli resimleri iþlemek istiyorsak, her pikselin H,S,V  deðerlerini
...
$\mu,\Sigma$ bu Gaussian'ýn ait olduðu bölge olacaktýr, $p$ boyuttur, yani
3. Türetime baþlamadan önce $\Sigma$'yi kestirme hesaplayan $\hat{\Sigma}$'yi
hatýrlayalým,

$$ \hat{\Sigma} = \frac{1}{n} \sum _{i=1}^{n} (x_i-\hat{\mu}) (x_i-\hat{\mu})^T $$

Kýsaltma amaçlý $C_j = 1 / \big((2\pi)^{k/2} \det(\Sigma_j)^{1/2}\big)$ diyelim, 
...
Þimdi aynen tek boyutlu örnekte olduðu gibi $\Sigma_0$ yerine onun kestirme
hesabýný formüle sokalým,
...
Bu formül nasýl kýsalabilir? Herhangi bir $\mu$ için $z_i=x_i-\hat{\mu}$
diyelim, $m_1+m_2$ yerine $n$ olsun, ve $z_i$ ifadesi $p \times 1$ boyutunda
vektörler. Genel olarak þu ifadeyi

$$ \sum_{i=1}^n z_i^T\left(\sum_{k=1}^n z_kz_k^T\right)^{-1}z_i $$

kýsaltmaya uðraþýyoruz. Burada iz operatörünü kullanabiliriz, iz bildiðimiz gibi
bir matrisin köþegeninin toplamýný verir. Güzel özellikleri vardýr, mesela
...
ile baþlayabiliriz. Ýz kullanabildik çünkü izini aldýðýmýz ``matris'' aslýnda
bir tek sayý. Þimdi izin üstteki ve toplam iþlemleri içine nüfuz edebilme
özelliðini kullanacaðýz,
...
Bölüm sýrasýnda $\exp$ terimleri iptal olur, sonuç
...
Tabii hesaptan önce üstteki formülde yine kestirme deðerleri yerine koyarak
hesabý yapacaðýz.

Renkli bir resme bakalým þimdi,
...
Görüldüðü gibi imaj matrisinde artýk her hücrede üç öðe var.
...
1. ve 2., sonra 1. ve 3. bölgeler arasýnda olurluk oranýný hesaplayalým,
...
Farklý bir resme bakalým, Alcatraz adasýnýn bir fotoðrafý mesela,
