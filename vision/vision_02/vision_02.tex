...
Yansýtma formülü (bkz {\em Lineer Cebir Ders 15}),
...
bir $\Sigma^{\dag}$'in köþegenine dizilir, ``sözde ters'' bu matris olur,

$$ \Sigma^{\dag} = \left[\begin{array}{cc}
\Sigma_1^{-1} & 0 \\ 0 & 0 
\end{array}\right]$$

Ve bu ters iþlemi tüm $A$'nin tersini almak için kullanýlýr, ki bu sözde
tersi de boyutu $n \times m$ olan bir $A^{\dag}$ ile gösteriyoruz
(Ýngilizce ``$A$-dagger'' olarak telafuz ediliyor, biz ``$A$-kama''
diyelim), 

$$ A^{\dag} = V \Sigma^{\dag} U^T  \mlabel{1}$$
...
$\Sigma^{-1}$ olmadýðý için yerine $\Sigma^{\dag}$ kullanýyoruz ve (1)'e eriþiyoruz.  
...
çünkü $x^* = A^{\dag}b$ ile hesaplanan çözüm ayný zamanda $|Ax - b|^2$'yi
...
giderek bizim bu derste kullandýðýmýz en güncel olan kitaba geliyor, {\em
