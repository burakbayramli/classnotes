...
O zaman Lagrangian'in minimum noktasi ustteki minimize ediciyi formule geri
sokunca elde edecegim sey
...
lineer ifadesini tatmin eden bir $x$ var. Bu durumda $x^*$'i

$$
x^* = -Q^{+}(c-u+A^Tv)
$$

olarak yazýlabilir, ki $^+$ iþareti genelleþtirilmiþ ters (generalized ýnverse)
demek. Bu iþleme bazen sözde ters (pseudo ýnverse) ismi de veriliyor. Bu
...
Ve $Q^+$ sözde tersini tanýmlamanýn bir yolu üstteki öz ayrýþtýrma
üzerinden (dikdörtgensel matrisler için SVD üzerinden) olur. Ama ondan önce
bir soru, eðer tüm deðerleri sýfýr olmayan bir $D$ üzerinden bir öz
ayrýþtýrmam olsaydý? Basit, $U D^{-1} U^T$ deðil mi? Unutmayalým $U$
matrisi dikgen. 
