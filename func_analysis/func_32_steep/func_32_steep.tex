...
olacak bazý cebirsel numaralar var. En basiti atýlan adým $\alpha$'yi
cebirsel olarak çözmek, altta bir örnek[3, sf. 101] var. 
...
üzerinden kok bulup bize döndürecek üstteki hesabý kullanabiliriz,
...
(first-order necessary condition -FONC-) minimumda $\nabla f(x) = 0$ olmasý. Eðer
böyle bir noktaya eriþmiþsek, diyelim $x^k$ için $\nabla f(x^k) = 0$ ölmüþ,
bu nokta FONC'yi tatmin eder çünkü o zaman $x^{k+1} = x^k$ olur, ve
minimumdayýz demektir. Bu teorik bilgiyi algoritmamizin ne zaman duracaðýný
anlamasý için bir þart olarak kullanamaz mýyýz? 
