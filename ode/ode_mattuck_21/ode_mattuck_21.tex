...
Kabaca yapýlan iþlemi þöyle tarif etmek mümkün: elimizde iki zamansal fonksiyon
var, bunlardan birini ters çeviriyoruz, ve her $t$ aný için bir düz, bir de ters
fonksiyon için, $g$ için sýfýrdan, $f$ için $t$'den baþlamak üzere, tüm
deðerlerin entegralini alýyoruz.

Ya da, her $t$ aný için $f$ üzerinde $t$ anýndan ileri / geri, $g$ üzerinde 0
anýndan geri / ileri giderek bu gidiþte elimize geçen deðerleri çarpýp
topluyoruz (ya da her iki serinin akýllý çarpýmýný alýyoruz, bkz {\em Entegralý
 Çarpým Olarak Görmek} yazýsý.

Ayrýksal olarak 

$$ f * g = \sum _{-\infty}^{\infty} f_{t-i} g_i  $$

Tabii sýnýrlar eldeki veri ile sýnýrlý olacak, eðer her iki fonksiyon için de
sýfýr öncesi tanýmlý deðilse, o zaman alt sýnýrý 0 olarak tanýmlayabiliriz,

$$ f * g = \sum _{0}^{\infty} f_{t-i} g_i  $$

Ayný durum entegralli sürekli formül için de geçerli.
