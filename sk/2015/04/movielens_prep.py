...
d = "/opt/Downloads/ml1m"
rnames = ['user_id', 'movie_id', 'rating', 'timestamp']
ratings = pd.read_table(d + '/ratings.dat', sep='::', header=None,names=rnames)
df2 = ratings.pivot_table('rating', index='user_id',columns='movie_id')
df2.to_csv("/tmp/out1.csv",sep=';')

import pandas as pd
unames = ['user_id', 'gender', 'age', 'occupation', 'zip']
users = pd.read_table(d + '/users.dat', sep='::', header=None,names=unames)
...
df3.to_csv(d + "/movielens.csv",sep=';',index=None)
