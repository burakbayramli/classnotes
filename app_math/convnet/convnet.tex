...
ilk dokunan katmaný sadece evriþim operasyonu için kullanmak. Bu ek'in
convnet'in alt katmanlarýndaki hata düzeltme iþlemini daha rahatlaþtýrdýðý
keþfedildi. Bir diðer katman özel bir ReLu (doðrultan lineer ünite
-rectified linear unit-) katmaný eklemek, bir diðeri ``aþaðý örnekleme
(downsampling)'' yapmak, mesela 2x2 içindeki bir pencere içine düþen
öðelerin maksimumunu almak, vs. Bir diðer katman ``veri atma (dropout)''
katmaný, veri içinde bir kýsým rasgele þekilde eðitim verisinden atýlýyor,
böylece modelde aþýrý uygunluk (overfitting) problemlerinden kaçýnýlmýþ
olunuyor, potansiyel olarak daha saðlam (robust) bir modelin ortaya çýkmasý
saðlanýyor.
...
parçasýnýn bile türevini alýnabiliyor (bkz {\em Otomatik Türev}
...
yaratmasý için oldukca çok veriye ihtiyaçlarý var, normal ölçek veri
...
hesaplarýn mesela güven aralýðýný hesaplamak mümkündür; Mesela týbbi bir
uygulama baðlamýnda YSA bir tahmin yapýnca bundan ``ne kadar emin olduðu''
sorusunun direk cevabý alýnamýyor. Bu alanda araþtýrmalar var tabii, ilginç
bazý buluþlar bunu mümkün kýlabilir.
