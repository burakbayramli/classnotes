...
# Implements a data structure that acts almost like a dictionary, with two
# modifications:
...
#        Each item is not removed until the next item is requested, so D[x] will
#        still return a useful value until the next iteration of the for-loop.
...
class priorityDictionary(dict):
   def __init__(self):
      ...
      entry will not remove the old pair from the heap until it
      is found by smallest() or until the heap is rebuilt.
      '''
      self.__heap = []
      dict.__init__(self)
   ...
   def __setitem__(self,key,val):
           ...
           pair to heap. Reb the heap if the number of deleted items
           gets large, to avoid memory leakage.'''
