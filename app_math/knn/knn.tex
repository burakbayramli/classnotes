...
\begin{document}
En Yakýn k-Komþu (k-Nearest Neighbor)
...
kolaylaþtýrýlmaya uðraþýlýr. Bu yapý belli bir noktaya en yakin k nokta
...
$||$ operatörü norm operatörü anlamýna gelir ve uzaklýk hesabýnýn
genelleþtirilmiþ halidir. Konu hakkýnda daha fazla detay için {\em
  Fonksiyonel Analiz} ders notlarý. Kýsaca söylenmek istenen iki nokta
arasýnda direk gitmek yerine yolu uzatýrsak, mesafenin artacaðýdýr. Tabii
uzaklýk, yol, nokta gibi kavramlar tamamen soyut matematiksel ortamda da
iþleyecek þekilde ayarlanmýþtýr. Mesela mesafe (norm) kavramýný
deðiþtirebiliriz, Öklitsel yerine Manhattan mesafesi kullanýrýz (blok
mesafesi, binalar etrafýndan dolaþýlýyor, direk gidiþ yok), fakat bu kavram
bir norm olduðu ve belirttiðimiz uzayda geçerli olduðu için üçgensel
eþitsizlik üzerine kurulmuþ tüm diðer kurallar geçerli olur.
...
\codeline /* Eðer alttaki þart geçerli ise node içindeki bir noktanýn daha önce \\
\codeline    keþfedilmiþ $k$ en yakýn komþudan daha yakýn olmasý imkansýzdýr*/\\
\codeline $\code{if } D^{node}_{minp} \ge D_{sofar}$ \\
\codeline \> $\code{return } PS_{in}$ deðiþmemiþ halde;\\
\codeline $\code{else if } node $ bir çocuk noktasý ise \\
...
\codeline \> \> \> en uzak olan komþuyu $PS^{out}$'tan çýkart;\\
\codeline \> \> \> $D_{sofar}$'i güncelle;\\
\codeline \\
\codeline /* eðer uç nokta deðil ise iki çocuk düðümden daha yakýn olanýný \\
\codeline    incele, sonra daha uzakta olanýna bak. büyük bir ihtimalle   \\
\codeline    arama devam ettirilirse bu arama kendiliðinden kesilecektir */ \\
\codeline $\code{else }$\\
\codeline \> $node_1 = node$'un $q$'ya en yakýn çocuðu;\\
\codeline \> $node_2 = node$'un $q$'dan en uzak çocuðu;\\
