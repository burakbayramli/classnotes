...
boyutlarýný ayný anda iþlerler. RNN için yapý þöyle deðiþiyor (tek bir
nöron için),

$$ y_t = \phi(x_t^T \cdot w_x + y_{t-1}^Tw_y + b)  $$

Ýlginç olan geçiþindeki $w_y,w_x$ aðýrlýk matrisleri, parametrelerinin her
zaman aný, her veri noktasý için ayný olmasý. Yani farklý zaman dilimleri
için farklý aðýrlýklar atanmýyor. $t$ anýndaki gizli (hidden) konum $h_t$,
bu bir nevi ``hafýza''. Bu fonksiyon $x_t$ girdisinin $W$ ile çarpýlmasý,
artý bir önceki konumun bir baþka $w_x$ ile çarpýlmasý sonucundan elde
ediliyor. $w_x,w_y$ matrisleri geçmiþe ne kadar önem verileceðini
tanýmlýyorlar. Ardýndan tüm hesap bir $\phi$ ile ``eziliyor'' yani belli
aralýklara düþmesi zorlanýyor, bunun için tipik olarak sigmoid, ya da
$tanh$ kullanýlýr.

Aynen normal YSA'da olduðu gibi bir ufak toptan veri parçasý (minibatch)
için tüm hesaplarý tek bir vektorize iþlemle yapabiliriz,

$$ Y_t = \phi(X_t^T \cdot W_x + Y_{t-1}^TW_y + b)  $$

$$  = \phi( 
\left[
  \begin{array}{cc} X_t & Y_{t-1} \end{array}
\right]
) \cdot W + b )   
\quad \textrm{ ki } \quad  W = \left[\begin{array}{c} W_x\\W_y \end{array}\right]
$$
...
RNN ismindeki ``tekrarlanma'' $W_x,W_y$'nin her zaman adýmý için ayný
...
adýmý için gösterebiliriz. Alttaki diagramlarda $\phi$ için $f$, $W_y$
yerine $W_h$ kullanýlmýþ. 0,1,.. indisleri ve apostrofi zaman dilimlerini
temsil için.

\includegraphics[width=8em]{rnndiag-0.jpg}
\hspace{3cm}
\includegraphics[width=19.5em]{rnndiag-1.jpg}

\includegraphics[width=19.5em]{rnndiag-2.jpg}
\includegraphics[width=19.5em]{rnndiag-3.jpg}

\hspace{1cm}
\includegraphics[width=25em]{rnndiag-4.jpg}

\hspace{1cm}
\includegraphics[width=25em]{rnndiag-5.jpg}

\hspace{1cm}
\includegraphics[width=25em]{rnndiag-6.jpg}

Bir diagram daha, daha önce gördüðümüz $h$ kafa karýþtýrmasýn, bir önceki
nöronun çýktýsý bir sonrakine girdi olmaktadýr, bir yandan ham veri girdisi
var tabii, bu þekilde o nöronun çýktýsý meydana gelir. Standart yapýda her
nöronun çýktýsý hesabý ve o çýktý için etiket olacaktýr, ve tahmin ile
çýktý arasýndaki uyuþmazlýk eðitim ile düzeltilmeye çalýþýlacaktýr,
vs. Gerçi bir önceki nörondan gelen veri ayrý aðýrlýklarla çarpýlýr, bu
sebeple direk veri gibi sonraki katmana girmez, bu açýdan o farklýlýðý
belirtmek için ``gizli'' þekilde tanýmlanabilir belki.

\includegraphics[width=25em]{rnn_02.png}
...
Ýlla her nöronun çýktýsýný kullanmak gerekmez. Her nöron çýktýsý bir
sonrakine girdi kabul edildiði için bu çýktýlar RNN'nin bütününü
etkilerler, fakat muhakkak etiket / düzeltme baðlamýnda direk
kullanýlmalarý þart deðildir. Mesela bir RNN'e bir veri dizisi verip,
çýktýlarýnýn en sonuncusu hariç geri kalanlarýný yok sayabiliriz, o zaman
bir dizi-vektör RNN'i elde ederiz. Mesela RNN'e bir film hakkýndaki tüm
yorumlarý kelime kelime veri dizisi olarak verebiliriz, RNN'in tek çýktýsý
-1/+1 þeklinde beðendi / beðenmedi skoru olabilir. Altta diyagramý
görüyoruz,
...
Dikkat, eðitim yapmadýk, RNN'e ileri yönde hesap yaptýrdýk, dört kez
$x_0,x_1$ verdik o da bize dört tane $y_0,y_1$ verdi. RNN'i eðitiyor
olsaydýk üretilen dört $y_0,y_1$ için ``gerçek'' veri, etiketler saðlamamýz
gerekecekti, onun üzerinden düzeltme yapacaktýk, vs.
...
çaðrý içindeki örnekleme kodunun benzerini altta tekrarlayalým ve grafikleyelim,
...
\begin{verbatim}
[[ 2.17743208  2.27357975  1.87228856  1.05163803]
 [-0.10520593 -0.90911746 -1.75240355 -2.39598168]
 [-0.80861926 -0.7867238  -1.23048937 -2.082059  ]
 [-1.37642148 -1.96414822 -2.65249624 -3.20183786]
 [-3.54107518 -3.90422668 -3.80257653 -3.37818064]]
\end{verbatim}
...
Öðrenme amaçýyla daha büyük adým, iç nöron sayýsý tanýmlayalým,
...
\begin{minted}[fontsize=\footnotesize]{python}
t_start = 29. # tahmine burada basla
n_more = 30 # bu kadar daha uret
...
Kýrmýzý renkli tahmin, yeþil renkli ne olduðugü. Tahminin alttan üste
çýkýþý biraz erken olmuþ ama genel gidiþatý tahmin edebilmiþ gözüküyor.
