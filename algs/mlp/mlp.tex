...
olarak $x_1,x_2,x_3$ ve çýktý olarak bir fonksiyon hesabý. Eðer 0/1,
...
olabilirdi, ya da düzeltilmiþ lineer ünite (rectified linear ünit -ReLu-)
...
sebeple tamamen baðlanmýþ katman (fully-connected layer) deniyor. Her türlü
girdi / nöron kombinasyonu çarpýldýðý için aðýrlýklar bir matris içinde
tutulur, ve aktivasyondan önceki vektör basit bir matris çarpýmý haline
gelir, mesela 2. katman aktivasyonu $f_2$ olsun, 1. gizli katman çýktýsý

$$ f_2 (W_1x + b) $$

Girdi $4 \times 1$, aðýrlýk $W_1$ boyutu  $3 \times 4$, o zaman çarpým $3
\times 1$ olacak, $f_2$ uygulanýnca boyut deðiþmez (her öge üzerinde ayný
fonksiyon iþletiliyor) sonuç $3 \times 1$, bu da zaten 2. katmana giren
(sonra oradan sonraki katmana çýkan) hesap. Üstteki diyagramda yanlýlýk
(bias) ayrý bir nöron olarak eklenmiþ, ama bazen bu yapýlmayabiliyor.

Iki gizli seviyesi (hidden layer) olan bir YSA,
...
eðitilirler. Elde $x_i,y_i$ veri noktalarý vardýr, ve her (potansiyel) çok
boyutlu $x_i$ yine (potansiyel) çok boyutlu $y_i$ ile eþlidir. Aynen
regresyonda olduðu gibi bu veri çiftlerini alýrýz ve onlarý en iyi temsil
eden YSA'yý eðitmek için kullanýrýz. Eðitim tamamlandýðýnda $w_i$
aðýrlýklarý belli deðerlere sahip olurlar, ve eldeki ``model'' bu
aðýrlýklardýr. Yeni bir veri noktasý verildiðinde o veri noktasý aðýrlýklar
$w_i$'lar ile çarpýlýp, toplanýp üstte görülen fonksiyonlara geçilip tüm
sonuçlar biraraya getirilir, ve çýktý deðeri böyle hesaplanýr.
...
Ýki gizli seviyeli YSA'larýn evrensel yaklaþýklayýcý (universal
approximator) olduðu iddia edilir, yani üstteki gibi yeterince çetrefil bir
YSA her türlü fonksiyonu yaklaþýk olarak temsil edebilir. Ýþin tarihine
gidersek, ilk baþta tek seviyeli YSA vardý, fakat XOR fonksiyonunu baþarýlý
þekilde temsil edemedi. Gizli (tek) seviyeli olan bunu baþardý.
...
boyutlu bir $x_i$ çýktý ise tek boyutlu 0,1,2,..,9 gibi etiketler.. Bazen
çýktýlar ikisel olarak modellenmek istenebilir, o zaman 0,..,9 yerine
onlarýn ikisel bazdaki halleri kullanýlýrdý, 4 bit yeterli, 9 sayýsý ikisel
olarak 1001'dir, o zaman çýktý $y_i$ dört boyutlu modellenir.
...
teknik var. YSA içinde, üstte gördüðümüz gibi, en üst seviyeden baþlayarak
dibe gidecek þekilde, fonksiyonlar, fonksiyonlarýn fonksiyonlarý þeklinde
...
Backprop'un genel bir resmi üstte: a) Eðitim verisi ileri yönde (solda
...
$$  
= (a_k-t_k)g_k'(z_k)\frac{\partial}{\partial w_{jk}} z_k \mlabel{1}
$$
