...
\codeline \> $E$ kenarlarýný $\pi = (o_1,..,o_m)$ þeklinde küçükten büyüðe doðru sýrala. \\
\codeline \> Ýlk baþta $S^0$ gruplamasýný al. Bu durumda her kenar $v_i$
kendi bileþeni içindedir. \\ 
\codeline \> $\code{for }$ $q = 1,..,m$ \\
\codeline \> \> $S^{q-1}$ gruplamasýný baz alýp $S^q$ gruplamasýný þöyle
yarat; $q$'inci sýradaki  \\
\codeline \> \> kenarýn birleþtirdiði noktalarýn $v_i,v_j$ olduðunu farz
edelim, yani $o_q = (v_i,v_j)$. \\ 
\codeline \> \> Eðer $v_i,v_j$ $S^{q-1}$ gruplamasý içinde farklý iki
bileþen içindeyseler, ve $w(o_q)$ her  \\
\codeline \> \> iki bileþenin içsel farkýna kýyasla çok küçük ise, bu iki
bileþeni birleþtir, \\ 
\codeline \> \> yoksa hiçbir þey yapma. \\
...
\begin{pseudocode}
\codename $\code{felzenswalb}\left(G\right)$\\
\codeline \> Bütün kenarlarý küçükten büyüðe doðru sýrala. \\
\codeline \> Ýlk baþta kenar $v_i$ kendi bileþeni içinde olsun, buna $S^0$ gruplamasý diyelim. \\ 
\codeline \> $\code{for}$ tüm kenarlar $e_i = (v_1,v_2) \in E$ için \\
\codeline \> \> $v_1 \in C_1$ ve $v_2 \in C_2$'nin birbirinden ayrý, ayrý bileþenler içinde \\
\codeline \> \>  ama ayný kenarý içeren noktalar olduðunu düþünelim, \\
\codeline \> \> $\code{if}$ $w(e_i) \le MInt(C_1,C_2)$ \\
\codeline \> \> \> $C_1$ ve $C_2$'yi birleþtir \\
\codeline \> \> $\code{else}$ \\
\codeline \> \> \> $S^i = S^{i-1}$ \\
...
Þimdi {\em SVD ile Kümeleme} yazýsýnda gördüðümüz kelime gruplamasý örneðini
Felzenswalb ile gruplayalým. 
