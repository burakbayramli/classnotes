...
çözüm one-hot kodlamasý, tüm dokümanlardaki tüm kelimeler on bin kelimelik
...
temsil eden sýnýrlý sayýdaki bir vektör YSA'nýn bir tabakasýna ``gömülür''
ve aynen YSA'nýn diðer katmanlarý gibi eðitilir. Baþlangýçta tüm
kelimelerin gömme vektörleri rasgele sayýlardýr, eðitim ilerledikçe bu
deðerler anlamlý hale gelirler.

Kodlama kabaca þöyle: tüm dokümanlar üzerinden sözlüðü oluþtururuz.
Kelimeler one-hot vektörü deðil, tek bir indis haline getirilir, üstteki
``cat'' sadece 300 sayýsýna dönüþür. Gömme tabakasý için sadece sýnýrlý
sayýda kelimeyi alýrýz, mesela ilk 5'i, yani her dokümanýn ilk 5 kelimesi
tutulur, gerisi atýlýr, eðer eksik varsa dolgulama (padding) ile sýfýrlar
eklenip 5'e getirilir. Tabii bu indis deðerleri YSA için direk
kullanýlamaz, bir sonraki aþama, YSA'ya bir gömme tabakasý eklemek, YSA'nýn
eðitimde kullanacaðý esas deðerler bunlar. Her kelimenin gömme boyutu $n$
önceden kararlaþtýrýlýr, mesela $n=4$ diyelim, eðer sözlük büyüklüðü $|V|$
ise, $n \times |V|$ boyutunda bir büyük gömme referans matrisi elde edilir,
matrisin baþlangýç deðerleri rasgeledir. YSA icinde bulunan gömme girdi
katmanýnýn tamamý 5 x 4 = 20 olacaktýr.
...
Girdi katmaný 5 x 4 boyutundaki bir ``tensor'' da olabilirdi (modern YSA
araçlarý çok boyutlu tensorlar ile rahatça çalýþýrlar), biz basitleþtirme
amaçýyla vektörün düzleþtirildiðini düþünelim,
...
tabakalarý gibi deðil; Her eðitim veri noktasý için indis bazlý esas
girdilere göre, {\em referans gömme matrisindeki} uygun satýrlar çekilip o
anda bir $x$ haline getiriliyor. Ardýndan geriye yayýlma ile YSA hata
düzeltme yapacaðý zaman gömme referans matrisindeki uygun vektörler
güncelleniyor.

Eðitim hedef deðeri nedir? Burada farklý yaklaþýmlar var, üstteki {\em
  Lineer Cebir} yazýsýnda komþu kelimeler tahmin edilmeye
uðraþýlýyordu. Bir baþka numara bir cümleyi alýp içindeki tek bir kelimeyi
``bozmak'', oraya anlamsýz bir kelime getirmek, ve bu yeni cümleyi 0,
bozulmamýþ olanýný 1 etiketi eðitmek. Cümleler nasýl olsa hazýr var, onlarý
bozmak kolay, bu þekilde iki kategorili bir sýnýflama problemi elde
ediyoruz. Bu yaklaþim için örnek boyutlarla, 
...
TensorFlow gömme tabakasý ile referans matrisi arasýndaki iliþkiyi
\verb!embedding_lookup! çaðrýsý ile yapýyor. Bu çaðrý aslýnda bir matriste
indis eriþimi sadece, 
...
Örnek olarak [2] verisi üzerinde üstte tarif edilen basit að yapýsýný
oluþturalým. Veri {\em Rotten Tomatoes} adlý film yorum sitesinden alýnan
kullanýcý yorumlarý (yazdýðý kelimeler yani) ve kullanýcýnýn filmi beðendi
/ beðenmedi þeklindeki hissiyatýný içeriyor. Bu bir doðal dil iþleme 0/1
sýnýflama problemi. Veriye bakalým,
...
Örnek seçtiðimiz bir kaç yoruma bakýyoruz (üstte), birincisi, ikincisi
pozitif, üçüncü negatif. Yorumlarda kullanýlan kelimelerde mesela ilkinde
``a good place to start (iyi bir baþlangýç noktasý)'' yorumu pozitifsel bir
hava taþýyor, üçüncüde ``uninventive (bir yenilik yok)'' kelimesi
kullanýlmýþ, negatif. YSA eðitildikten sonra bu kelimelerin doðru temsilsel
(gömme) aðýrlýklarýný ve onlarýn 0/1 hedefine olan baðlantýsýný öðrenmeyi
umuyoruz. 
...
Ýndisler \verb!x! kullanýmý kafa karýþtýrmasýn, [1] kodlamasý o þekilde
seçmiþ. 

Ardýndan gömme yapýlýr, ve bu aðýrlýklar \verb!fully_connected! ile tam
baðlamýþ YSA tabakasýna verilir, oradan çýkan sonuç ise iki kategorili
...
n-gram iþlemi bu eski yaklaþýmlarda çoðunlukla bir öniþlem (preprocessing)
aþamasýnda veriden yeni veri çýkartarak yapýlýrdý, DYSA ile elimizde daha
kullanýþlý bir silah var. Evriþim.

Altta görülen örnekte gömme tabakasý $9 \times 6$ boyutunda, bu tabaka
...
diðeri (sarý renkli). Böylece ikinci seviyedeki vektörler elde ediliyor, bu
vektörler üzerine max-pool iþlemi uygulanýyor, ve oradan gelen sonuçlar
softmax'e veriliyor. 

\includegraphics[width=35em]{nlp_01.png}

Bu þekilde deðiþtirilmiþ kod altta. 
...
1. kodu iþletin, ve gömme referans matris aðýrlýklarýna bakýn. Örnek bir
kelime seçip o kelimenin gömme matrisindeki diðerlerine olan mesafesini
