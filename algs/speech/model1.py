...
class Model:
    def __init__(self):
        ...
        self.stfts = tf.contrib.signal.stft(self.data, frame_length=256, frame_step=128, fft_length=256)
        ...
        for _ in range(num_layers):
            ...
            cell = tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=1-self.dropout_prob)
            cells.append(cell)
        ...
        self.softmax = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits,labels=self.y) 

        self.cross_entropy = tf.reduce_mean(self.softmax)

        self.train_step = tf.train.AdamOptimizer(0.001).minimize(self.cross_entropy)
        
        self.correct_prediction = tf.equal(tf.argmax(self.y,1), tf.argmax(self.logits,1))
