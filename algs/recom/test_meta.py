...
with zipfile.ZipFile('ml-100k.zip', 'r') as z:
    all_ratings = pd.read_csv(z.open('ml-100k/u.data'), sep='\t',names=["user_id", "item_id", "rating", "timestamp"])
    items = pd.read_csv(z.open('ml-100k/u.item'), sep='|', encoding='latin-1',names=names)
