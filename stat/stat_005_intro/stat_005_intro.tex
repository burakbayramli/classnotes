...
$k=2$. $\Sigma$ pozitif kesin (positive definite) bir matristır. Hatırlayalım,
bir matris pozitif kesindir eğer tüm sıfır olmayan $x$ vektörleri için
$x^T\Sigma x > 0$ ise.
...
Devam edersek, eğer $\Sigma$ pozitif kesin ise bir $\Sigma^{1/2}$ matrisini
olduğu gösterilebilir, ki bu matrise $\Sigma$'nin karekökü ismi verilir, ve
bu karekökün şu özellikleri vardır, 1) $\Sigma^{1/2}$ simetriktir, 2)
