...
yazýda türetildi. Mesela {\em Çok Deðiþkenli Calculus Ders 9}, {\em Lineer
...
notasyon istatistikteki ``tahmin edici (estimatör)'' notasyon ile
...
Ýdeal durumda, yani modelin doðru, veriye uyduðu durumda artýklarýn mükemmel bir
Normal daðýlýma sahip olmasý gerekir, çünkü veri içindeki tüm örüntü, kalýp
model tarafýndan ``bulunmuþtur'' ve geri kalanlar gürültüdür (gürültü tabii ki
Normal daðýlýmda). Ýdeal ortamda OLS algoritmasýnýn, matematiksel olarak,
ortalamasý (mean) sýfýr olan artýklar üretmesi garantidir. Bir diðer varsayým
uyduralan deðiþkenlerin katsayýlarýnýn onlarýn ``gerçek'' deðerleri etrafýnda
merkezlenen bir Normal daðýlýma sahip olduðudur (ispat için {\em Tahmin
  Aralýklarý} yazýsýnýn sonuna bakýlabilir). Bu normallik önemli çünkü katsayý
tahmini ile standart hatayý bölünce baþka bir Öðrenci t daðýlýmý ortaya çýkacak.

Kalýntýlarýn normalliði QQ grafiði ile kontrol edilebilir (bkz {\em Güven
  Aralýklarý, Hipotez Testleri} yazýsý,
...
karekökü alýrsak, bu rasgele deðiþkenin Chi Kare (Chi Square) olarak daðýldýðý
bilinir, ve yine bilinir ki standart normal rasgele deðiþken, bolu, chi square
karekökü bize bir Öðrenci t daðýlýmýný verir, mesela
...
biraz daha ek iþ gerektiriyor, türetilmesi {\em Tahmin Aralýklarý (Prediction
  Interval)} bölümünde.
