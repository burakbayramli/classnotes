...
def user_movie():
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            skey = "%s-%s" % (user_id, movie_id)
            if hash(skey) % 100 == 0:
                test_set[user_id][movie_id] = rating
            else:
    ...
    with open(output_test_file, 'w', encoding='utf-8') as f:
        for user_id in sorted(test_set.keys(), key=int):
            ratings_json = json.dumps(test_set[user_id])
            f.write(f"{user_id}|{ratings_json}\n")
