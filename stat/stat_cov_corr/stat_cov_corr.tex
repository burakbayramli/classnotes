...
Cauchy daðýlýmýnýn $f_X(x) = \{ \pi (1+x^2) \} ^{-1}$ olduðunu söylemiþtik. Þimdi 
beklentiyi hesaplayalým. Parçalý entegral tekniði lazým, $u=x$, 
$dv = 1/1+x^2$ deriz, ve o zaman $v = \tan ^{-1}(x)$ olur, bkz {\em Ters
  Trigonometrik Formüller} yazýmýz. Demek ki 
...
elde olan  $(x_1,y_1),(x_2,y_2),..$ verilerini kullanarak hem $x=j1$ hem
$y=1$ olan verileri sayýp toplamý $n_{11}$'e yazarýz, ayný þekilde diðer
kombinasyonlara bakarak alttaki tabloyu oluþtururuz [5], 
...
{\sqrt{n_{0\bullet} n_{1\bullet} n_{\bullet 0} n_{\bullet 1}}} \mlabel{6}
$$

ile hesaplanýr. Bu ifadeyi türetmek için korelasyon formülü ile baþlýyoruz, 
...
Sadece 1,0 deðerlerine sahip olan $X,Y$ Bernoulli daðýlýmýna sahip
deðiþkenler düþünelim, o zaman 
...
$E(XY)$ nasýl hesaplandý? Ayrýksal daðýlýmlar için beklenti formülünün iki
deðiþken için þöyle ifade edildiðini görebiliriz,

$$  E[XY] = \sum_i\sum_j x_i\cdot y_j \cdot P\{X = x_i, Y = y_j\} $$

Bu ifadeyi tabloya uyarlarsak, iki üstteki sonucu elde edebileceðimizi
görürüz, çünkü $x_i y_i$ sadece her iki deðer ayný anda 1 olduðunda
geçerlidir. Bu degerleri yerine gecirince (6) elde edilir.

Phi katsayýsýnýn bir diðer ismi Matthews korelasyon katsayýsý. Bu katsayý
hesabýný mesela bir 0/1 tahmini üreten sýnýflayýcýnýn baþarýsýný ölçmek
için kullanabiliriz, gerçek, test 0/1 verileri bir dizinde, üretilen
tahminler bir diðerinde olur, ve Phi katsayýsý ile aradaki uyumu
raporlarýz. Sonuç 0,1 arasýnda (ya da -1,+1) olacaðý için sonuca bakarak
irdeleme yapmak kolaydýr. 

Phi hesabýnýn, AUC hesabý gibi, dengesiz veri setleri üzerinde (mesela 0'a
kýyasla çok daha fazla 1 olan veriler, ya da tam tersi) hala optimal olarak
þekilde çalýþtýðý [4] bulunmuþtur. Bazý örnekler,
