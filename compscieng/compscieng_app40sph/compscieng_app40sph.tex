...
yapabilmemizi saðlayacak bir metot kullanmak. SPH tekniði bunu saðlýyor.
...
SPH'in temel fikri entegral aradeðerlemeden geliyor, hatta ona bir tür Çekirdek
Yoðunluk Kestirme (Kernel Density Estimation) yöntemi de denebilir. Sývýyý
ayrýksal parçalara bölüyoruz, ve takip edilen ölçüt ne ise çekirdek fonksiyonu
üzerinden bu ölçüt pürüzsüzleþtiriliyor / etrafa yayýlýyor / yumuþatýlýyor. Yani
çekirdek yarýçapý içindeki diðer parçacýklar o an üzerinde olduðumuz parçacýðýn
neyini ölçüyorsak onu etkiliyor, mesela yakýndaki parçacýklarýn basýncý ya da
yoðunluðu bizim basýncý, yoðunluðu etkiliyor. Kabaca düþünülünce aslýnda bu
bakýþ açýsýnýn sývýlarýn tabiatýna son derece uygun olduðu anlaþýlacaktýr.

Yaklaþýklamayý sývýya uygulamak için onu kütle öðelerine bölmemiz gerekli,
$i$'inci öðe $m_i$ kütlesine, $\rho_i$ yoðunluðuna, ve $r_i$ pozisyonuna sahip
olacak. $A$ ölçütü / hesabýnýn $i$ parçacýðýndaki deðeri $A_i$ ile
gösterilsin. Her parçacýk için,
...
Formülde kütle kullanýmý daha faydalý çünkü kütle simülasyona baþlamadan önce
bilinir, hatta çoðu parçacýk için birbirinin aynýsýdýr. 

Üstteki toplamýn teoride tüm parçacýklar üzerinden alýndýðý gösterildi, fakat
pratikte $r$'ye yakýn olan diðer parçacýklar üzerinden alýnýr, çoðunlukla
çekirdek $2h$ gibi bir uzaklýkta zaten hýzla sýfýra düþtüðü için, ayrýca
performans açýsýndan, çok uzaktaki parçacýklarý göz önüne almaya gerek yoktur.

Bu formül bana herhangi bir $r$ noktasýnda hesaplayabileceðim bir $A$ kestirme
hesabý (estimation) vermiþ oldu. Parcaçýk bazlý simulasyon yaptýðýmýz için her
$i$ parçacýðý çevresindeki $j$ parçacýklarý üzerinden bu kestirme hesabý
yapabileceðiz.

Bazý hesaplar formülün basit yan ürünü, mesela yoðunluk
...
Yani etraftaki parçacýklarýn kütlesi üzerinden alýnan bir aðýrlýklý ortalama
bana parçacýðýn yoðunluðunu veriyor.

Aradeðerlemenin bize saðladýðý güzel bir özellik onun üzerinden mesela kolayca
gradyan alabilmem. Diyelim ki $A$ ölçümü basýnç, yani $p$. O zaman (1)'de
görülen $-\nabla p$ formülünü üstteki entegral üzerinden açabilmem gerekir ($p$
ölçümünün ya da hesabýnýn nereden geldiði ileride anlatýlacak). $r$'de olan her
$i$ parçacýðý için $p(r_i)$ diyelim,
...
böylece iþi daha kolaylaþacaktýr.
...
Yanlýz burada bir problem var, $j$ bölgesindeki basýncý kullanarak o fark
...
Basýnç kuvveti eðer basýnç biliniyorsa hesaplanabilir, basýncýn kendisini
hesaplamak için Ýdeal Gaz Kanunu gerekli [9], bu kanun

$$
p V = n R T
$$

ki $V = 1 / \rho$, yani birim kütleye düþen hacim, $n$ mol üzerinden kaç tane
gaz parçacýðý olduðu, $R$ evrensel gaz sabiti, $T$ ise sýcaklýk. SPH için sývýda
eþsýcaklýk (isothermal), ve parçacýklarýn sayýsý simulasyon sýrasýnda deðiþmez
kabul edilir, o zaman üstteki formülün sað tarafýnda bir gaz sertlik (stiffness)
sabiti $k$ kullanýlabilir [4, sf. 18], 
...
Sayýsal kodlamalarda stabilite amaçlý olarak ayrýca bir de $\rho_0$ kullanýlýr,
ki her basýnç hesabý için yoðunluðun bu baz $\rho_0$ ile olan farký bazlý
alýnýr, nihai formül,
...
Simülasyon kodu alttadýr. Simüle edilen bir blok sývýyý havada asýlý bir
durumdan býrakmak. Ne olacak? Yerçekim etkisiyle tabii ki sývý parçacýklarý
düþmeye baþlayacaklar, ama normal sadece elastik çarpýþma içeren simülasyondan
farklý olarak þimdi sývý parçacýklarý arasýnda aðdalýk var, yani birbirlerine
yakýn durmaya meyilliler. Ayrýca basýnç var, tüm bunlarýn etkisini simüle eden
program acaba sývýsal bir davranýþ gösterecek mi? Simülasyon animasyon / video
olarak [10]'da bulunabilir, C++ versiyonu [11]'de, derleme bilgileri [12].
...
[9] Bayramlý, {\em Fizik, Temel Fizik 3, Basýnç, Çarpýþma}

[10] Bayramlý, {\em Simulasyon Animasyonu},
    \url{https://github.com/burakbayramli/classnotes/blob/master/compscieng/compscieng_app40sph/balls4.gif?raw=true}

[11] Bayramli, {\em 3D SPH C++ Versiyonu},
    \url{https://raw.githubusercontent.com/burakbayramli/classnotes/master/compscieng/compscieng_app40sph/simsph.cpp}

[12] Bayramli, {\em SPH Pürüzleþtirilmiþ Parcaçýk Hidrodinamiði (Smoothed Particle Hydrodynamics -SPH-)
     \url{https://burakbayramli.github.io/dersblog/sk/2020/08/sph.html}
