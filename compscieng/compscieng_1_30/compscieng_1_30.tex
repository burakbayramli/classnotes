...
{\em Diferansiyel Denklemler Ders 15}'te sürekli Fourier Transformunu
gördük,

$$ X(f) = \int _{-\infty}^{\infty} x(t) e^{j2\pi ft} \ud t $$

ki $x(t)$ zaman bazlý sürekli bir sinyaldýr. Bilgisayarlar sayesinde
...
Üstteki formülden anlaþýldýðý üzere DFT aslýnda $2\pi$'i $N$ parçaya
bölerek önceden belirli bu frekanslar üzerinden katsayý hesabý yapýyor,
...
bu hýzda ardý ardýna alýnmýþ 8 tane veri noktasý olsun, yaný
$N=8$. Ayrýksal formül,
...
kullanýlacak, ki formül suna benzer,
...
Bu sonuçlarda 1. sayý hayali sayýnýn reel kýsmý, 2. sayý pür kompleks
kýsmý. Çoðunlukla bu elde edilen $X(m)$ sayýsýnýn büyüklüðü (magnitude) ile
...
Not: Örnekte sürekli bir formülden baþlayarak $x(n)$ hesabý yapýlmýþ olmasý
ayrýksallýðýn nerede baþlayýp bittiði hakkýnda kafa karýþýklýðý
yaratmasýn; analitik formül iki yöre arasýnda gidiþ / geliþ nasýl oluyor
bunu göstermek için yapýldý. Çoðunlukla elimizde sadece bir tek boyutlu bir
dizi içinde bir seri veri olur, bu verinin hangi analitik formülden
geldiðini bilmeyiz (Fourier analizinin onun için yapýyoruz zaten!), veri
hakkýnda tek (gerekli) bilgi onun örneklem frekansý olacaktýr, $x(n)$ bize
þeriden $n$'inci veriyi döndürmekten baþka bir þey yapmaz.

Bir ornek uzerinde daha gorelim,
...
$k=2,5$'te büyüklük var, bu deðerler $k\omega_0 = 2\pi k / NT$ ile $\pi$
bazina cevirilebilir, $k=2$ icin $2\pi k / 3.2 = 1.25 \pi$, $k=5$ icin
$3.125 \pi$ elde ederiz.

Ders 15

[Strang hocanýn notlarýndan. Üstteki DFT hesabý, toplamlar, döngüler
içinde, belli indiþler için yapýldý. Altta Strang hocanýn anlattýðý ilginç
bir yaklaþým var, sinyalin frekanslarýnýn katsayýlarla aðýrlýklanmýþ
toplamý olduðu temel Fourier formülü ile baþlarýz, fakat diðer yöne gitmek
için pür lineer cebir kullanýrýz, böylece katsayý hesabý bir matris tersi
alma iþlemi haline gelir. Altta bu yaklaþým anlatýlýyor].
...
Benek verisindeki periyodik oluþ, Fourier transformu ile analiz etmek için
...
algoritmayý kullanýr, örnek altta:
