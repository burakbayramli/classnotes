...
{\em Diferansiyel Denklemler Ders 15}'te surekli Fourier Transformunu
gorduk, 

$$ X(f) = \int _{-\infty}^{\infty} e^{j2\pi ft} \ud t $$

ki $x(t)$ zaman bazli surekli bir sinyaldir. Bilgisayarlar sayesinde
ustteki islemi ayriksal olarak yapabiliyoruz [2, sf. 46], 

$$ X(m) = \sum _{n=0}^{N-1} x(n) e^{-j2\pi nm / N} $$

$x(n)$ her $n$ anýnda, bir $n$ indisiyle eriþilen bir anlamda $x(t)$'den
``örneklenmiþ'' sinyal verisi olacaktýr, $e$ tabii ki doðal logarýtma bazý,
$j$ ise  hayali sayý $j = \sqrt{-1}$. 

Euler iliskisi $e^{-j\phi} = \cos(\phi) - j\sin(\phi)$'den hareketle,
ustteki ayriksal denklem suna esittir, 
