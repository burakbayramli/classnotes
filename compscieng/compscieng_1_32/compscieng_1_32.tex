...
[3, sf. 247]], öyle ki sýfýrda ortalanmýþ ve $-\omega,\omega$ arasýndaki
bir bölgede fonksiyon 1, diðer yerlerde 0, yani
...
olarak görebiliriz. Bu fonksiyonu bir sinyal ile evriþtirince ona frekans
filtresi uygulamýþ oluruz, sadece $f_c$ altýndaki frekanslara izin veren
alçak geçiren (low-pass) filtreyi elde ediyoruz [2, sf. 285].

Tek problem $\sinc$ fonksiyonunun sýfýr genliðe eriþmeden eksi ve artý
sonsuzluða sürekli devam etmesi. Bu sonsuz uzantý sayýsal ortamda
kullanýþlý deðil. Bunun için fonksiyonu bize sýfýrda ortalanmýþ $M+1$ tane
katsayý deðeri býrakacak þekilde kýrpacaðýz ($M$ çift sayý olmalý, sifirda
ortalaninca geri kalan katsayilardan her iki yönde eþit sayýda olmasý
için), ve geri kalan tüm $\sinc$ deðerleri sýfýr kabul edilecek. 

Bir diðer problem sýfýrda ortalama demek negatif indiþler kullanmak demek,
...
\includegraphics[width=30em]{compscieng_1_32_15.png}
...
$$ w[i] = 0.54 - 0.46 \cos (2\pi 2 / M)  $$

Nihai filtre

$$ 
h[i] = K \frac{\sin(2\pi f_c (i-M/2))}{i-M/2} \bigg[
0.54 - 0.46 \cos (2\pi 2 / M)
\bigg]
$$

\includegraphics[width=30em]{compscieng_1_32_16.png}

Kodlarý altta görüyoruz, 

\inputminted[fontsize=\footnotesize]{python}{filt.py}

Alçak Geçiren (Low-Pass) Filtre
...
Yüksek Geçiren (High-Pass) Filtre

Yüksek geçiren filtre elde etmek için herhangi bir $f_c$ için tanýmlý bir
alçak geçiren filtreyi alýp, katsayýlarýný negatife çevirmek yeterli (bir
de ortada sýfýr noktasýnda bir pozitif deðer ekleniyor). Bu deðiþim o
filtreyi $f_c$ için yüksek geçiren hale getirir.
...
Bant Geciren (Band-Pass) Filtre

Sadece $f_{c1}$ ve $f_{c2}$ frekanslarý arasý haricindeki tüm diðer
frekanslar elensin istiyorsak $f_{c1}$ için alçak geçiren $f_{c2}$ için
yüksek geçiren filtre tanýmlayýp bu iki filtreyi toplarýz. Bu toplam bize
bir bant reddi (bant-reject) filtresi verir. Ama biz bunun tersini
istiyoruz, o zaman eldeki katsayýlarý üstteki bahsedilen yöntemle tersini
alýrsak, bir bant geçiren filtre elde ederiz. 
