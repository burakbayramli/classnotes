...
filtrelenmiþ sinyali üretiyorduk,

$$ y[n] = a_0 x[n] + a_1 x[n-1] + a_2 x[n-2] +  a_3 x[n-3] + ... $$

Aslýnda filtreleme için ham sinyalin kendisi elimizdeki tek girdi seçeneði
deðil, sinyal $x$ ile beraber geriye dönük þekilde $y$'nin kendisini de
...
bir ortalama filtremiz var, ve $n=50$'de $y[50]$ þu deðere sahip,

$$ y[50] = x[47] + x[48] + x[49] + x[50] + x[51] + x[52] + x[53] $$

Sinyalde $n$ ortalanarak 3 geri ve 3 ileri gidilerek toplam alýnýyor,
filtrelenmiþ sinyal bu. Þimdi ayný filtrenin bir sonraki zaman kesitinde,
$y[51]$'deki deðerine bakalým,
...
Simdi daha onceki genel formu hatirlayalim 

$$ y(n) + \sum_{k=1}^N a_k y(n-k) = \sum_{l=0}^M b_l x(n-l) $$

Bu formül bir farklýlýk denklemi. Literatürde daha önce (5) formülü aslýnda
transfer fonksiyonu $H(z)$ olarak geçiyor,
...
Bu formun güzel tarafý eðer transfer fonksiyonuna bir þekilde eriþebilirsek
oradan direk farklýlýk denklemine geçebiliriz, katsayýlar $a_0,..,a_n$ ve
...
dürtü deðiþmezliði (impülse ýnvariance) yöntemi. Bu yöntem ile bilinen bir
analog filtrenin analog transfer fonksiyonu $H(s)$'i alýr, sonra ters
Laplace transformu ile dürtü cevap fonksiyonu $h(t)$'yi elde eder. Sayýsala
geçmek için bu fonksiyon belli aralýklarla örneklenir, yani $h(t)$, $h(nT)$
haline getirilir. Sonra bu fonksiyonun z-transformu alýnarak $H(z)$'ye
geçiþ yapýlýr, bu geçiþ bize özyineli fonksiyonumuzda kullanabileceðimiz
direk katsayýlarý verecektir. 

Analog deyip geçmeyelim, alçak geçiren, yüksek geçiren, vs. filtreleme
konusunda burada derin bir literatur var. Sayýsal yöntemler ortaya
...
(hale de yapýyor), mesela alttaki devre çok basit bir alçak geçiren
filtredir. 
...
filtrelenmiþ olarak çýkacaktýr. Bu klasik bir filtre devresi ve bir tranfer
fonksiyonu var. 


\begin{minted}[fontsize=\footnotesize]{python}
fs = 10.0 # orneklem orani
T = 10.0 # saniyelik sinyal
n = int(T * fs) 
t = np.linspace(0, T, n, endpoint=False)
data = np.sin(0.40*2*np.pi*t) + 1.5*np.cos(9*2*np.pi*t) + 0.5*np.sin(12.0*2*np.pi*t)
plt.plot(t,data)
