...
{\em Matris Çarpýmý} adlý yazý tek makinalý ortamda matris çarpýmýnýn nasýl
yapýlacaðýný, ve nasýl görülecebileðini anlattý. Satýr bakýþ açýsý, kolon bakýþ
açýsý iþlendi. Parallel (Hadoop), eþle/indirge ortamýnda matris çarpýmýný nasýl
yaparýz?  Mesela $A^TA$'yi ele alalým. Bu çarpým oldukça önemli çünkü baþka
sonuçlar için de kullanýlabiliyor. Mesela $A$ üzerinde $QR$ ayrýþtýrmasý yapmak
isterseniz (bkz. Lineer Cebir ders notlarýmýz) bu çarpým kullanýlabiliyor.
...
boyunca deðerleri olan bir matristir. Daha fazla detay için bkz. {\em
  Lineer Cebir Ders 29}. Þimdi $A = QR$ yerine koyalým,
