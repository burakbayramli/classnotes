...
PCA durumunu yeni veriyi kullanarak güncelliyoruz. Örnek için Ýris veri
setinde görelim, 

\begin{minted}[fontsize=\footnotesize]{python}
import pandas as pd, incpca

df = pd.read_csv('../../stat/stat_preproc/iris.csv')
df = np.array(df)[:,:4].astype(float)
pca = incpca.IncPCA(n_components=2)
S  = 10
df2 = df.copy()
for i in range(15):
    fr = i*S; to = (i+1)*S
    print 'Ufak parca', fr,to,'isleniyor'
    pca.partial_fit(df2[fr:to, :])
print 'aciklanabilen varyans', pca.explained_variance_
print 'varyans orani',pca.explained_variance_ratio_
print 'essiz degerler', pca.singular_values_
...
\begin{verbatim}
Ufak parca 0 10 isleniyor
Ufak parca 10 20 isleniyor
Ufak parca 20 30 isleniyor
Ufak parca 30 40 isleniyor
Ufak parca 40 50 isleniyor
Ufak parca 50 60 isleniyor
Ufak parca 60 70 isleniyor
Ufak parca 70 80 isleniyor
Ufak parca 80 90 isleniyor
Ufak parca 90 100 isleniyor
Ufak parca 100 110 isleniyor
Ufak parca 110 120 isleniyor
Ufak parca 120 130 isleniyor
Ufak parca 130 140 isleniyor
Ufak parca 140 150 isleniyor
aciklanabilen varyans [ 0.23764206  4.04160534]
varyans orani [ 0.4381091   7.45097096]
essiz degerler [  5.97045296  24.6219577 ]
sonuc [[ 0.64411706  0.36431173]
 [ 0.74129597 -0.08044427]
 [-0.17490763  0.85571279]
 [-0.07071643  0.35855446]]

aciklanabilen varyans [ 4.19667516  0.24062861]
varyans orani [ 0.92461621  0.05301557]
sonuc [[ 0.36158968  0.65653988]
...
[9] Ross, {\em Incremental Learning for Robust Visual Tracking}, \url{http://www.cs.toronto.edu/~dross/ivt/RossLimLinYang_ijcv.pdf}
