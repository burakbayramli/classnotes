...
gerçekleþtirilir, daha fazla detay için {\em Lineer Cebir 33}'e bakýnýz)
...
Ýteratif iþlem sözde kod olarak,

\begin{algorithm}[h]
\begin{pseudocode}
\codename $\code{imputed\_svd}$\\
\codeline $\code{while }$ $z$'deki deðiþim azalýncaya kadar (convergence)\\
\codeline \>  $z = VV^Ty$  \\
\codeline \>  $y$'nin ilk halindeki bilinen noktalarý alýp $z$'ye kopyala  \\
\end{pseudocode}
\end{algorithm}
