...
direðin üzerinde bir kütle var; acaba bu direði sadece arabaya uygulanan
bir $F$ kuvveti ile saða sola götürerek dengeleyebilir miyiz? Belki
bazýlarýmýz elimiz üzerinde bir sopayý dengelemeye uðraþmýþýzdýr, yapmaya
çalýþacaðýmýz buna çok benziyor.
...
$v_1$ arabanýn hýzý, $v_2$ ise sarkacýn hýzý. $x(t)$ arabanýn yerini
...
gerekiyor, sarkacýn yeri nedir? Onun yatay, dikey kordinatlarýna bakalým,
...
eþitliðin sað tarafýndan dahil edilebilir. Ayrýca hýza doðru oranda ters
...
Örneðe dönelim; Noktalý kýsýmlarý atarsak 1. dereceden bir yaklaþýksallama
ve lineerizasyon elde ediyoruz ve görülen $x^*,y^*$ noktasýndaki deðerleri
kullanýlan 2x2 matrisi Jacobian matrisidir.
...
(1)'den elde edilen matrisin 1. ve 2. satýrý sýrasýyla alttaki noktalý
yerlere gelecek,
...
Görüldüðü gibi $x_1,x_2$'nin ne olduðu basit. $x_3,x_4$ için (1)
denklemindeki matris iþlemlerini yapýp 1. ve 2. satýrlarýný $x_3,x_4$ için
kullanabiliriz. Önce (1) formülündeki gerekli $x_i$ deðiþken deðiþimlerini
yaparýz,
...
Ýki matris satýrý elde ettik, bunlarý yerine koyalým,
...
Bir gayrý-lineer ODE elde etmiþ olduk. Þimdi yapmak istediðimiz bu sistemi 
...
haline sokmak, yani lineer bir þekilde temsil edebilme, örnekte birinci
denklem $\dot{x} = Ax(t) + B u(t)$ yeterli. Lineerizasyonu kritik nokta
yakýnýnda yapacaðýz, o zaman LQR adlý bir teknik sistemin o noktada kalmasý
için gerekli kontrol $u$ deðerini hesaplayabiliyor.
...
Örneðe dönelim; (2) matrisinin Jacobian'inin kritik nokta $(0,0,0,0)$'daki
deðerini bulabiliriz. Jacobian'ýn ana matris 1. ve 2. satýrý için alýnan
kýsmý türevleri basit,
...
3. ve 4. satýr Jacobian iþlemleri de \verb!sympy! ile yapýlabilir. Ýlginç
...
Bu deðerleri $J_x$'deki yerlerine koyarsak,
...
Ardýndan $F$ için Jacobian, yine 1. ve 2. satýr atlanýyor, (2)'nin 3. ve
4. satýrlarý için kýsmý türev,
...
Böylece $(0,0,0,0)$ kritik noktasý etrafýnda lineerize edilmiþ sistemimiz
için bir $\dot{x} = A x + B u$ denklemi elde etmiþ olduk, 

Lineer Karesel Düzenleyici (Linear Quadratic Regulators -LQR-)

Sistemi belirtilen kritik nokta etrafýnda lineerize etmemizin bir sebebi
vardý; çünkü bir lineer sistem varsa, o sistem herhangi bir denge noktasý
etrafýnda LQR ile tutulabilir. $(0,0,0,0)$ noktasý çubuðun tam dik olduðu
(açý orada sýfýr) yerdir, amacýmýz çubuk dengelemek, o zaman dengelemeyi
burada yapabiliriz. Eðer sistem bu duruma yakýnsa (çubuðun tam aþaðý düþmüþ
olduðu durumlarda bu yaklaþým ise yaramaz), LQR kullanýlabilir. Tam taným;
baþlangýcý $x_0$ olarak bilinen bir
...
$u$ deðerini bulmaktýr. Çözüm $u(t) = -K(t) x (t)$, ve $K$ cebirsel
Riccati denklemi üzerinden bulunur [2]. Hesapsal pek çok kütüphanede bu
çözümü yapacak \verb!lqr! çaðrýlarý vardýr. $Q,R$ ile $x,u$ içindeki hangi
deðiþkenlere daha önem, aðýrlýk vereceðimizi tanýmlayabiliriz, mesela iki
boyutlu durumda köþegen matriste 
...
tanýmlamak $x_1$'e $x_2$'ye göre iki kat daha önem verildiðini gösterir. 

O zaman gerekli $u(t)$'yu bulduktan sonra bu aksiyonu sisteme
uygulayabiliriz, yani $\dot{x} = A x + B u$ sistemini bulunan $u$ ile
entegre ederiz, ve bu sistem dengeye giden bir çubuk olacaktýr. Altta bu
çözümü, baþlangýç acýsý $\theta = 0.5$ ve $-5$ için görüyoruz. Sonuçlar
...
Kodlar genel hatlarýyla [4]'u baz almýþtýr. 

Özyineli bir þekilde LQR yapan bir diðer kod [7]'de, \verb!ilqr.py! dosyasýnda
bulunabilir.

Ödev

[4] baðlantýsýnda paylaþýlan kodda bir ``yukarý fýrlatma (swing-up)''
tekniði görülüyor. Bu teknik direðin tam aþaðý düþmüþ olduðu durumlarda
kullanýlabilir, (üst denge noktasýndan uzaktayýz, bu sebeple mevcut
sistem ise yaramaz), arabaya hýzlý ve belli bir þekilde sað-sol hareketi
yaptýrarak direði ``yukarý fýrlatýyoruz''. [4]'teki kod baðlantýsýný takip
edip Matlab kodlara bakarak aynýsýný üstteki koda uygulayýn. 
