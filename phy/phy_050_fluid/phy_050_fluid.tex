...
Þimdi ilginç bir noktaya geldik, süreklilik denklemi (1)'i, $\rho$ ölçümü
üzerinde materyel türev uygulanmýþ formu olarak görmek mümkün,

$$
\frac{D \rho}{D t} + \rho \nabla \cdot \bar{u} = 0
$$

Pürüzsüz Parcaçýk Dinamiði (Smoothed Particle Dynamics -SPH-)

Sývýnýn akýþýný simüle etmek pek çok alanda faydalý olabilecek bir
...
oyunlarda duman simülasyonuna kadar giden geniþ yelpazede teknikler kullaným
bulabilir.

SPH tekniði bu sývý simülasyon yöntemlerinden birisidir. Navier-Stokes
denklemleri bir sývýnýn akýþýný tanýmlar, bu denklem simülasyon ortamýnda
yaklaþýk olarak çözülmelidir [5], bunu yapmanýn iki yolu vardýr. Önceki
anlatýmdan hareketle herhalde cevabý tahmin edebiliriz, birisi Lagrange usulü
diðeri Euler usulü. SPH yöntemi Lagrange yaklaþýmýný takip eder, yani bir sývý
paketini alýp onun zaman ve uzayda gidiþini izler. Tek bir yere mýhlanmýþ
olmamanýn (Euler usulü) pek çok avantajlarý var, mesela oyun grafiklerini
çizerken daha geniþ bir alanda iþ yapabilmek mümkün, farklý maddeler arasýndaki
iletiþim daha rahat halledilebiliyor, vs.
...
edebiliriz, oradan kuvvet, enerji baðlantýlarý var, $F=ma$ formülünden
...
dalan herkes etrafýndaki suyun basýncýný bir kuvvet olarak hisseder. O zaman,
...
\bar{g}$. Ývme $\bar{g}$ bir vektör olarak gösterildi, bu $\bar{g} = (0, 0,
...
Evet. Temel Calculus'tan hatýrlarsak gradyan en dik iniþ yönünü gösterir, o
zaman gradyanýn negatifi yüksek basýnç bölgesinden dýþarý, alçak basýnç
bölgesine doðru gösterecektir. Tabii ki bu etkiyi üzerinde olduðumuz sývý
paketinin hacmi üzerinden entegre etmemiz gerekir, ama basit bir yaklaþýklama
olarak onu direk hacim $V$ ile çarpabiliriz. Tümü $F^{\textrm{basýnç}} = - V
\nabla p$.

Üçüncü kuvvet aðdalýk. Aðdalý bir sývý deforme edilmeye karþý direnir. Bu
kuvveti kabaca ``her parçacýðý yanýndaki diðer parçacýklarýn ortalama hýzýnda
...
çalýþanlar bir ölçünün etrafýndaki ortalamadan ne kadar saptýðýný hesaplamalak
için bir diferansiyel operatör kullanýr, bu operatör Laplasyan $\nabla \cdot
\nabla$.  Bu operatör, ayný þekilde hacim üzerinden entegre edilince bize
aðdalýk kuvvetini verebilir, burada bir ek her sývýnýn farklý aðdalýðýný formüle
dahil edebilmek için bir de bir dinamik aðdalýk sabiti (dynamic viscosity
coefficient) $\mu$ ile herþeyi çarpmak gerektiði, o zaman $F^{\textrm{aðdalýk}}
...
düþünürsek daha iyi olur, $m/V$'yi sývý yoðunluðu $\rho$ olarak tanýmlayalým,
...
Basitleþtirme amacýyla kinematik aðdalýk $\nu = \mu / \rho$ tanýmlayalým, o
zaman 
