...
Açısal hız en basit halde alttaki şekilde hareketle anlaşılabilir, $\theta$
açısının katettiği çembersel mesafe $r\theta$'dir, eğer zamansal $\theta(t)$
biliniyorsa, $\omega = \dot{\theta}$ bize açısal hızı, ve $v = r\omega$ ise
teğetsel hız $v$'yi verir.
...
Çok boyutlu dönüşsel ortamda momentumun ve kuvvetin karşılığı açısal
momentum ve torktur, çok boyutlu ortamda bunları alttaki resimde görebiliriz,

\includegraphics[width=25em]{phy_005_basics_13.png}
...
ki $r$ referans $O$'ya göre olan yer vektörü, ve $v$ hız, $mv$ tabii ki
momentum olur. Buradan hareketle $O$ seçiminin açısal momentum hesabını
etkileyeceğini anlayabiliriz. Mesela bir kuyruklu yıldız güneşin yörüngesinde
olduğu zaman bu dönüşün açısal momentumu güneşin merkezine göredir [5, sf. 81].
...
Bu hesap ta yine $O$ referanslı, $r = \vec{OA}$. Eğer $r$'yi bir vida sıkıştıran
pensenin kolu gibi düşünürsek vida $O$'da duruyor, ona uygulanan tork üstteki
$T$ ile hesaplanıyor. 
...
[5] Levi, {\em Classical Mechanics with Calculus of Variations and Optimal Control}
