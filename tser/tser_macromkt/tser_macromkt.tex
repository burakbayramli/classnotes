...
eþitliðin bir tarafýnda tek baþýna býrakmak analitik olarak mümkün deðil. Ama
sayýsal kök bulma yöntemlerini kullanabiliriz. Bu yöntemlerden Newton'un
yöntemini {\em Diferansiyel Denklemler, Kök Bulmak, Karesel Formül} yazýsýnda
gördük. Bir $f(x)$ için $f(x)=0$ sonucunu verecek $x$ deðerlerini bulmak kök
bulmaktýr. Tabii üstteki problemde sýfýra deðil, belli bir sabit deðere olan
eþitlik var, $f(x) = a$ gibi, ama bu önemli deðil, bu problemi sýfýra eþitliði
baz alan kök bulmaya çevirebiliriz, $g(x) = a-f(x) = 0$ ile mesela. Ardýndan
Newton yöntemini yine olduðu gibi, $g$ üzerinde kullanýrýz,
