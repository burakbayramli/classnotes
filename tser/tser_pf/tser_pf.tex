...
Üstteki entegrali $x_{0:t} | y_{1:t}$'den örneklem alarak ve entegrali toplam
haline getirerek yaklaþýksal þekilde hesaplayabileceðimizi {\em Ýstatistik,
  Monte Carlo, Entegraller, MCMC} yazýsýnda gördük. Fakat $x_{0:t} |
y_{1:t}$'den örnekleyemiyoruz. Bu durumda yine ayný yazýda görmüþtük ki
örneklenebilen baþka bir daðýlýmý baz alarak örneklem yapabiliriz, bu tekniðe
önemsel örnekleme (importance sampling) adý veriliyordu. Mesela mesela herhangi
bir yoðunluk $h(x_{0:t})$ üzerinden,
...
Bu formülde yeþil renkli kýsma bakalým, bu aslýnda (3)'e göre $w_{t-1}$'in
tanýmý deðil mi?
...
\codeline 
$t=1$ yap ve $x_0^{(1)}, ..., x_0^{(N)} \sim \pi(x_0)$ örneklemini al \\
\codeline 
\code{for } $i=1,..,N$ için \\
\codeline \>
...
\codeline \>
Önemsel aðýrlýklar $\tilde{w}_t^{(i)} = g(y_t|\tilde{x}^{{i}})$'ý hesapla. \\
\codeline \>
$N$ tane yeni parçacýk $(x_{0:t}^{(i)}; i=1,..,N )$ eski parçacýklar \\
\codeline \>
$\{ \tilde{x}^{(i)}_{0:t},...,\tilde{x}^{(i)}_{0:t} \}$ içinden normalize edilmiþ önemsel\\
\codeline \>  aðýrlýklara göre örnekle. \\
\codeline \>
$t = t + 1$ \\
\codeline \>
\code{goto } 2. satýr
...
bakýnýz {\em Yapay Görüþ, Obje Takibi} yazýsý. 
